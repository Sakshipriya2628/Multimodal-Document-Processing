...
def describe_images(folder_path):
    ...
    if data:
        ...
        if not df.empty:
            excel_path = os.path.join(os.path.abspath(os.getcwd()), 'CIPD9 Analysis.xlsx')
            df.to_excel(excel_path, index=False, engine='openpyxl')
            logging.info(f"Data successfully written to {excel_path}")
        else:
            logging.warning("DataFrame is empty. No data to write to Excel.")
    else:
